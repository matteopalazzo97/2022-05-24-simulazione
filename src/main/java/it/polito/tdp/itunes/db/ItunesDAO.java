...
public class ItunesDAO {
		...
		try {
			...
			while (res.next()) {
				result.add(new Track(res.getInt("TrackId"), res.getString("Name"), 
						...
						res.getInt("Bytes"),res.getDouble("UnitPrice"), res.getInt("GenreId")));
	...
	public List<Track> getVertici(int genreID){
	...
	public List<Arco> getArchi(int genreID, Map<Integer, Track> mappa){
		final String sql = "SELECT DISTINCT t1.`TrackId` AS track1, t2.`TrackId` AS track2, abs(t1.`Milliseconds`-t2.`Milliseconds`) AS peso "
				+ "FROM (SELECT * "
				+ "		FROM track t "
				+ "		WHERE t.`GenreId`=?) AS t1, "
				+ "	(SELECT * "
				+ "		FROM track t "
				+ "		WHERE t.`GenreId`=?) AS t2 "
				+ "WHERE t1.`MediaTypeId` = t2.`MediaTypeId` "
		...
		try {
			...
			while (res.next()) {
				result.add(new Arco(mappa.get(res.getInt("track1")), mappa.get(res.getInt("track2")),
						res.getInt("peso")));
	...
	public Map<Integer, Track> getMappaTracks(){
		final String sql = "SELECT * FROM Track";
		...
		try {
			...
			while (res.next()) {
				result.put(res.getInt("TrackId"), new Track(res.getInt("TrackId"), res.getString("Name"), 
						res.getString("Composer"), res.getInt("Milliseconds"), 
						res.getInt("Bytes"),res.getDouble("UnitPrice"), res.getInt("GenreId")));
